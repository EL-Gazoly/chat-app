...
const FriendsView = ({width} : {width : number}) => {
    ...
    const pendingFirends = friends?.filter((friend) => friend.status === "pending");
    ...
    return ( 
        <div className=' flex flex-col mt-5 px-4 gap-y-7'>
            ...
            <div className=' flex flex-col  gap-y-4'>   
                    ...
                    {
                pendingFirends?.map((friend) => {
                    return <PendingCard key={friend.userId} width={width} friend={friend} />
                })
            }
            </div>
            <div className=' flex flex-col gap-y-4'>   
