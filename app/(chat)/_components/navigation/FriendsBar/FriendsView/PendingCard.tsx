...
export const PendingCard = ({width, friend}: PendingCardProps) => {
    ...
    const accpetFriendRequest = useMutation(api.firends.accpetFriendRequest);
    const declineFriendRequest = useMutation(api.firends.rejectFriendRequest);
    const handleAccept = () => {
        accpetFriendRequest({friendRequest: friend._id, friendId: id});
    }
    const handleDecline = () => {
  ...
  { width > 315 &&  <div  className=' flex items-center gap-x-4' >
